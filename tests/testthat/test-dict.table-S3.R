context("dict.table S3")

test_that("dict.table creation works as expected", {
    dat = data.table(A = 1, B = 2)
    dit = dict.table(A = 1, B = 2)
    expect_equivalent(dit, dat)
})

test_that("access of dict.table properities work as expected", {
    dat = data.table(A = 1, B = 2)
    dit = dict.table(A = 1, B = 2)
...
test_that("a column can be addded to a dict.table if name is specified as character", {
    dit = dict.table(A = 1)
...
test_that("a column cannot be added twice", {
    ...
    expect_error(add(dit, "A", 1:2), "key 'A' already in dict.table")
})
...
test_that("deleting non-existing columns gives an error", {
    ...
    expect_error(delete(dit, 3), "Column '3' out of range (ncol = 2)", fixed = TRUE)
    expect_true(identical(dit2, dit.copy))
})

test_that("if delete operation fails, dict.table object remains untouched", {
    dit = dict.table(A = 1:2, B = 2:1)
    dit.copy = data.table::copy(dit)
...
test_that("dict.table can be printed", {
    dit = dict.table(A = 1:2, B = 2:1)
    expect_output(print(dit),
...
test_that("dict.table has a size", {
    dit = dict.table(A = 1:2, B = 2:1, C = 3:4)
    expect_equal(size(dit), ncol(dit))
    expect_equal(size(clear(dit)), 0)
})

test_that("a copy of the underlying data.table object of a dict.table can be retrieved", {
    dat = data.table(A = 1:2, B = 2:1, C = 3:4)
    dat2 <- as.data.table(dat)
    dit = dict.table(dat)

    dit.dat = values(dit)
    expect_true(inherits(dit.dat, "data.table"))
    expect_true(inherits(dit, "dict.table")) # verifies that dit.dat is a copy
})

test_that("standard dict getter works as expected", {
    dit = dict.table(A = 1:2, B = 2:1, C = 3:4)
    daf = as.data.frame(dit)

    expect_equal(getval(dit, "A"), daf[["A"]])
    expect_error(getval(dit, c("A", "B")), "key must be of length 1")
})

test_that("keys are the same as colnames", {
    dit = dict.table(A = 1:2, B = 2:1, C = 3:4)
    expect_equal(keys(dit), colnames(dit))
})

test_that("peek works as expected", {
    dit = dict.table(A = 1:2, B = 2:1)
    expect_equal(peek(dit, "A"), getval(dit, "A"))
    expect_error(peek(dit, c("A", "B")), "key must be of length 1")
...
test_that("a column can only bet set if already existing unless declared to be added", {
    dit = dict.table(A = 1)
    expect_error(setval(dit, "B", 1), "key 'B' not in dict.table")
...
test_that("the length of set column must match or be of length 1", {
    dit = dict.table(A = 1:4, B = 4:1)
    expect_error(setval(dit, "A", 1:3), "Supplied 3 items to be assigned to 4 items of column 'A'")
...
test_that("a column can be set by numeric index", {
    ...
    expect_error(setval(dit, 3, 1:2), "column '3' not in dict.table")
})

test_that("column names can be sorted increasingly and decreasingly", {
    dit = dict.table(A = 1:2, B = 2:1)
    daf = as.data.frame(dit)
...
test_that("rbind works as expected for dict.tables", {
