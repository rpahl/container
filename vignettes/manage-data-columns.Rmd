...
columns with minimal required additional checking resulting in very lean code.
...
remains the way to go when speed is key, but note that again there is no
checking of missing or existing data columns, as implicitly provided by the
`dict.table` operations.
...
checking, thereby yielding safer and less redundant code and
ultimately freeing the developer from writing these annoying checks over and
over again.
