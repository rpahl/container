...
```
| Container              | Deque         | Set            | Dict                       | Dict.frame     |
| :----------------------| :-------------| :--------------| :--------------------------| :--------------|
| Container\$new()       | Deque\$new()  | s <- Set\$new()| Dict\$new()                | Dict.frame\$new() |
|                        |               |                |                            |                |
...
|                        | peek()        | is.equal(s)    | peek(key, default = NULL)  |                |
|                        | peekleft()    | is.subset(s)   | pop(key)                   |                |
|                        | pop()         | is.superset(s) | popitem()                  |                |
|                        | popleft()     | union(s)       | rename(old, new)           |                |
|                        | reverse()     |                | set(key, value, add=FALSE) | set(key, value, add=FALSE) |
|                        | rotate(n=1L)  |                | sort(decr=FALSE)           |                |
|                        |               |                |  update(other)             |                |
|                        |               |                |                            |                |
|                        |               |                |                            | dim()          |
|                        |               |                |                            | ncol()         |
...
For more details visit the respective online helps (see `?Container`, `?Deque`, `?Set`, `?Dict`, and `?Dict.frame`). Usage examples are found in the corresponding [Container](Container.html), [Deque](Deque.html), [Set](Set.html), [Dict](Dict.html), and [Dict.frame](Dict.frame) vignettes.
...
| container                    | deque               | set              | dict                         | dict.frame         |
| :----------------------------| :-------------------| :----------------| :----------------------------| :------------------|
|  co = container()            |  d = deque()        |  s = set()       |  d = dict()                  | dif = dict.frame() |
|                              |                     |                  |                              |                    |
|  add(co, elem)               |                     |  add(s, elem)    |  add(d, key, val)            |                    |
|  clear(co)                   |                     |                  |                              |                    |
|  discard(co, elem, right=F)  |                     |                  |  discard(d, key)             |                    |
|  empty(co)                   |                     |                  |                              |                    |
|  has(co, elem)               |                     |                  |  has(d, key)                 |                    |
|  print(co, list.len=10)      |                     |                  |                              |                    |
|  remove(co, elem, right=F)   |                     |                  |  remove(d, key)              |                    |
|  size(co)                    |                     |                  |                              |                    |
|  type(co)                    |                     |                  |                              |                    |
|  values(co)                  |                     |                  |                              |                    |
|                              |                     |                  |                              |                    |
...
|                              |  popleft(d)         |  s1 + s2         |  setval(d, key, val, add=FALSE)  | setval(d, key, val, add=FALSE) |
|                              |  reverse(d)         |                  |  sortkey(decr=FALSE)         |                    |
|                              |  rotate(d, n=1L)    |                  |  update(d, other)            |                    |
|                              |                     |                  |                              |                    |
|                              |                     |                  |                              |  as.data.frame(dif)|
|                              |                     |                  |                              |  colnames(dif)     |
|                              |                     |                  |                              |  dim(dif)          |
