...
The container has a print method
```{r}
print(collection)
```

Alternatively, the internal representation can be retrieved directly using the `values` function.
```{r}
collection$values()
```

If initialized with an R object, the type of the objected is adopted to allow for efficient internal representations, if required.
```{r}
...
tryCatch(ages$popitem(), error = function(e) e$message)
```
