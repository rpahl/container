...
|Iterable | Container                | Deque           | Set              | Dict                         | Dict.frame        |
|---------|--------------------------|-----------------|------------------|------------------------------|-------------------|
| iter()  |                          |                 |                  |                              |
...
|         |  values()                |                 |                  |                              |
|         |                          |                 |                  |                              |
|         |                          |  addleft(elem)  |  diff(s)         |  get(key)                    | ncol()
|         |                          |  count(elem)    |  intersect(s)    |  keys()                      | nrow()
|         |                          |  peek()         |  is.equal(s)     |  peek(key, default=NULL)     | rownames()
|         |                          |  peekleft()     |  is.subset(s)    |  pop(key)                    | set_rownames()
|         |                          |  pop()          |  is.superset(s)  |  popitem()                   |
|         |                          |  popleft()      |  union(s)        |  set(key, value, add=FALSE)  |
...
More details about the methods are found in the respective online helps (see `?Container`, `?Deque`, `?Set`, `?Dict`, and `Dict.frame`).
